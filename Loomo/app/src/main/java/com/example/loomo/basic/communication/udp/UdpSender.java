...
public class UdpSender extends Thread{
	...
	/**
	 * Send a String to host computer
	 * @param msg
	 */
	...
	/**
	 * Method to send Loomo state to Loomo
	 * @param state LoomState
	 */
	public void send(LoomState state){
		byte[] data = state.toJsonBytes();
		DatagramPacket sendPacket = new DatagramPacket(data, data.length, address, port);
	...
	/**
	 * Method to send a bitmap to host computer
	 * @param bitmap
	 */
	public void send(Bitmap bitmap){
		int byteNum = bitmap.getByteCount();
		ByteBuffer buffer = ByteBuffer.allocate(byteNum);
		bitmap.copyPixelsToBuffer(buffer);

		DatagramPacket sendPacket = new DatagramPacket(buffer.array(), buffer.capacity(), address, port);
